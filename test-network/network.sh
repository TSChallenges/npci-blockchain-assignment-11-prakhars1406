...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating TataMotors Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-TataMotors.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
    ...
    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-ICICIBank.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
    ...
    infoln "Creating Tesla Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-Tesla.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
    ...
    infoln "Creating ChaseBank Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-ChaseBank.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.example.com docker_Tesla.org1.example.com docker_peer0.TataMotors.example.com docker_peer0.ICICIBank.example.com docker_peer0.ChaseBank.example.com 
